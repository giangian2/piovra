...
class StockLevelRepositoryAdapterTest extends PiovraIntegrationTest {
    ...
    void lock_or_create_returns_a_zeroed_level_for_an_unknown_sku() {
        ...
        StockLevel level = inTransaction(() -> adapter.lockOrCreate(TenantId.DEFAULT, sku));
    ...
    void ensure_exists_is_a_true_no_op_on_a_second_call() {
        ...
        StockLevel first = inTransaction(() -> adapter.lockOrCreate(TenantId.DEFAULT, sku));
        adapter.save(new StockLevel(TenantId.DEFAULT, sku, 5, 0, 0, first.version() + 1));

        adapter.ensureExists(TenantId.DEFAULT, sku);
        StockLevel afterSecondEnsure = inTransaction(() -> adapter.lockOrCreate(TenantId.DEFAULT, sku));
    ...
    void save_persists_the_new_level() {
        Sku sku = Sku.of("TEST-" + Ids.newId());
        StockLevel before = inTransaction(() -> adapter.lockOrCreate(TenantId.DEFAULT, sku));

        adapter.save(new StockLevel(TenantId.DEFAULT, sku, 42, 0, 0, before.version() + 1));

        StockLevel after = inTransaction(() -> adapter.lockOrCreate(TenantId.DEFAULT, sku));
